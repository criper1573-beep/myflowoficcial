...
### 2a. Telegram API (Telethon — чтение истории чатов)

**Назначение:** Выгрузка истории сообщений из чатов/каналов через Telethon (MTProto). Bot API историю не выдаёт — нужен пользовательский аккаунт.
...
**Где получить:**
- https://my.telegram.org/apps — войти по номеру, создать приложение
- api_id (число) и api_hash (строка) — после создания

**Используется в:**
- `blocks/telegram_chat_reader/` — чтение истории, мониторинг новых сообщений
- MCP-инструмент `telegram_chat_history`

**Мониторинг канала** (ловит новые сообщения, анализирует через GRS AI, шлёт уведомления):
...
```
Ввести номер телефона и код из Telegram. Сессия сохраняется в `storage/telegram_sessions/`.
