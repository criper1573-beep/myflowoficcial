...
5. **Разрешить перезапуск сервисов без пароля (для пользователя, от которого крутится вебхук):**  
   ...
   **Кнопки «Запустить»/«Остановить» в дашборде:** если дашборд запущен от **root** (User=root в analytics-dashboard.service), кнопки работают без настройки sudoers. Если дашборд запущен от другого пользователя, ему нужны права без пароля на `systemctl start` и `systemctl stop` для всех юнитов из Диспетчера задач. Пример (подставь пользователя и полный список юнитов):  
   `u123 ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart analytics-dashboard, ... , /usr/bin/systemctl start ИМЯ_ЮНИТА, /usr/bin/systemctl stop ИМЯ_ЮНИТА, ...`  
   (для каждого юнита: start и stop). Либо переведи сервис analytics-dashboard на User=root.
