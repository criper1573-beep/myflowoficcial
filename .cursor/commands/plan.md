...
- **If creative phases identified:** предложи пользователю запустить `/creative` (сам не переходи к креативу без явного запроса).
- **If no creative phases:** предложи пользователю запустить `/build` или написать **build** / **билд**, когда будет готов к реализации.

**Важно:** после PLAN фаза завершена. Не начинай реализацию (не меняй код) до тех пор, пока пользователь явно не попросит выполнить build (напишет **build**, **билд**, **реализуй** или вызовет `/build`).
